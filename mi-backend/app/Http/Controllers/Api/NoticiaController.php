...
class NoticiaController extends Controller
{
    ...
    public function store(Request $request)
    {
        ...
        $id = DB::table('noticias')->insertGetId([
            ...
            'tipo' => $request->tipo,
            'created_by' => auth()->id(),
            'publicado' => true, // Aseguramos que estÃ© publicado
            'created_at' => now(),
            'updated_at' => now()
        ]);
    ...
    if (!empty($tokens)) {
        ...
        $message = CloudMessage::new()
            ->withNotification(Notification::create(
                '[' . ucfirst($request->tipo) . '] ' . $request->titulo, // Tip: Ponemos la categorÃ­a en el tÃ­tulo visual
                substr($contenidoPlano, 0, 100) . '...'
            ))
            // AGREGAMOS ESTO: Datos ocultos para la navegaciÃ³n
            ->withData([
        ...
        Log::info("NotificaciÃ³n enviada con datos de navegaciÃ³n.");
    }
} catch (\Exception $e) {
